...
targets=$(psql -d blackosprey -t -A -c "SELECT target FROM temp_segments_${tt} WHERE ST_Intersects(geom, (SELECT ST_Buffer(ST_ExteriorRing(geom)::geography, 0.000001)::geometry FROM avaricosa_point_1km_buffer WHERE primary_key = '$pk'));")
