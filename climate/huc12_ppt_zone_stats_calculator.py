...
if not len(sys.argv) == 3:
	print "Usage: huc12_ppt_zone_stats_calculator.py image zone_raster hucid"
	print "Example: huc12_ppt_zone_stats_calculator.py /Volumes/BlackOsprey/GIS_Data/PRISM/4km/us_ppt_1895.01.tif mask_2020004.tif 2020004"
	print
	sys.exit(1)
...
for i in range(0,out.shape[0]):
	...
	sql = "INSERT INTO prism_ppt_statistics_huc12 (huc_12, prism_year, prism_month, num_pixels, mean, std_dev, sum)  VALUES (%s,%s,%s,%s,%s)" % (int(float(o[0])), pyear, pmonth, int(float(o[1])), ','.join(o[2:]))
	cursor.execute(sql)
