...
if not len(sys.argv) == 4:
	print "Usage: huc8_tmax_zone_stats_calculator.py image zone_raster temp"
	print "Example: huc8_tmax_zone_stats_calculator.py /Volumes/BlackOsprey/GIS_Data/PRISM/4km/temperature/tmax/us_tmax_1895.01.tif mask_2020004.tif tmax"
	print
	sys.exit(1)
...
for i in range(0,out.shape[0]):
	...
	sql = "INSERT INTO prism_%s_statistics (huc_8_num, prism_year, prism_month, num_pixels, mean, std_dev, max, min)  VALUES (%s,%s,%s,%s,%s)" % (temp, int(float(o[0])), pyear, pmonth, int(float(o[1])), ','.join(o[2:]))
	cursor.execute(sql)
