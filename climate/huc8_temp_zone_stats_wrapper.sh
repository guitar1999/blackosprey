...
huclist=$(psql -d blackosprey -t -A -c "SELECT huc_8_num FROM prism_unprocessed_${temp};")
rasterlist=$(ls /Volumes/BlackOsprey/GIS_Data/PRISM/4km/temperature/${temp}/*tif)

for huc in $huclist
do
	# Dump the watershed
	pgsql2shp -f huc_${temp}_${huc}.shp blackosprey "SELECT huc_8_num, ST_Transform(geom, 4322) FROM nhd_hu8_watersheds WHERE huc_8_num = '$huc';"
