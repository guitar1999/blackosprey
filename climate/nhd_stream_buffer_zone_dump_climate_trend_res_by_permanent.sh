...
echo $huc
# Dump the watershed
pgsql2shp -f ${edir}/avaricosa_buffer_${huc}_climate_trend.shp blackosprey "SELECT 1 as dumpid, ST_Transform(ST_Union(ST_MakeValid(geom_buffer)), 4326) AS geom FROM public.nhd_flowline_all_no_duplicates WHERE permanent_ = '$huc';"
