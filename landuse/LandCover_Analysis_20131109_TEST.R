...
#and "percent_change".
hbars <- function(lcc, field, main.title, xlab, colors) {
  ...
  #plot settings
  par(mar=c(5,8,4,2))
  xlims <- range(pretty(c(min(lcc[[field]]), max(lcc[[field]]))))
  #colors <- c("blue4", "cyan", "khaki2", "darkgreen", "springgreen2", "tomato2","skyblue1")

  #main <- paste(main.title, ":", "\n", sub.title, sep="")
  barplot(lcc[[field]], horiz=TRUE, legend.text=T, names.arg=lcc$class, las=1, 
...
polyfile <- "/Users/tcormier/Google Drive/wicklow/brook_floater/testing/buffer_analysis/NY_7818_poor_buff1000m.shp"
...
outfile <- "/Users/tcormier/Google Drive/wicklow/brook_floater/testing/buffer_analysis/test_NY_landuse_change_Poor_NY_7818.pdf"
...
for (i in c(1:length(poly))) {
  ...
  #titles and such - **FIX titles to be automated for either change or land use in 2001 - right now, manual.
  ...
  hbarplot <- hbars(lcc, "percent_change", poly.main, "Percent Change", colors)
  dev.off()
