...
def huc_bounds(huc, cursor):
    ...
    query = """SELECT ST_XMin(geom) AS llx, ST_YMin(geom) AS lly, ST_XMax(geom) AS urx, ST_YMax(geom) AS ury FROM (SELECT ST_Transform(ST_Union(ST_MakeValid(buffer_geom)), 5070) AS geom FROM avaricosa_buffer_table WHERE primary_key = '{0}') AS tquery;""".format(huc)
...
def rasterize_huc(huc, edir, coordlist):
    ...
    command = """/Library/Frameworks/GDAL.framework/Programs/gdal_rasterize -a dumpid -of GTiff -a_nodata 0 -te {0} {1} {2} {3} -tr 30 30 -ot Byte -co "COMPRESS=LZW" {4}/avaricosa_buffer_{5}_clip_vector.shp {4}/avaricosa_buffer_{5}_clip.tif""".format(xmin, ymin, xmax, ymax, edir, huc)
    proc = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    procout1 = proc.communicate()
    for f in glob.glob("{0}/avaricosa_buffer_{1}_clip_vector.*".format(edir, huc)):
        os.remove(f)
...
def summarize_huc(huc, edir, coordlist, cursor, db):
    ...
    # Read the huc
    huc_handle = gdal.Open("{0}/avaricosa_buffer_{1}_clip.tif".format(edir, huc))
    ...
    query = "UPDATE avaricosa_buffer_summary_statistics SET (num_pixels) = ({0}) WHERE primary_key = '{1}'".format(num_pixels, huc)
    cursor.execute(query)
    db.commit()
    ...
    for year in files.keys():
        ...
        for dt in files[year]:
            ...
            if dt == "impervious" or dt == "cd":
                ...
                query = "UPDATE avaricosa_buffer_summary_statistics SET (s{0}_{1}_mean, s{0}_{1}_std_dev, s{0}_{1}_min, s{0}_{1}_max) = ({2}, {3}, {4}, {5}) WHERE primary_key = '{6}';".format(year, dt, mean[0], std_dev[0], zmin[0], zmax[0], huc)
            else:
                ...
                if cols:
                    query = "UPDATE avaricosa_buffer_summary_statistics SET ({0}) = ({1}) WHERE primary_key = '{2}';".format(','.join(cols), ','.join(vals), huc)
                else:
...
def main(huc, edir, cursor, regen):
    ...
    if not os.path.isfile("{0}/avaricosa_buffer_{1}_clip.tif".format(edir, huc)) or regen:
...
# Parse the arguments
p = argparse.ArgumentParser(prog="avaricosa_buffer_summary_statistics.py")
