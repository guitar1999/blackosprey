    ...
    for year in files.keys():
        ...
        for dt in files[year]:
            ...
            if dt == "impervious" or dt == "cd":
                ...
                query = "UPDATE huc_12_summary_statistics SET (s{0}_{1}_mean, s{0}_{1}_std_dev, s{0}_{1}_min, s{0}_{1}_max) = ({2}, {3}, {4}, {5}) WHERE huc_12 = '{6}';".format(year, dt, mean[0], std_dev[0], zmin[0], zmax[0], huc)
            else:
                ...
                query = "UPDATE huc_12_summary_statistics SET ({0}) = ({1}) WHERE huc_12 = '{2}';".format(','.join(cols), ','.join(vals), huc)
            #print query
            cursor.execute(query)
            db.commit()
