...
raster='/Volumes/BlackOsprey/GIS_Data/USGS/NED/1arcsec_processed/ned_study_area_1arcsec.img'
for huc in $huclist
do
	echo $huc
	# Dump the watershed
	pgsql2shp -f /Volumes/BlackOsprey/GIS_Data/NHD/hu12_rasters/huc_${huc}_ned.shp blackosprey "SELECT dumpid, geom FROM nhd_hu12_watersheds WHERE huc_12 = '$huc';"
	...
	gdal_rasterize -at -a DUMPID -a_nodata 0 -init 0 -te $ll $ur -tr $tr -co "COMPRESS=LZW" huc_${huc}_ned.shp mask_${huc}_ned.tif

	# Clean up the shapefile
	/bin/rm huc_${huc}_ned.*
